...
public class CurrentICHelper {
     ...
     * @return
     */
    public List<VanillaOption> createCondorsOptions(Quote quote,double standardDeviation,double bound){
        List<VanillaOption> list = new ArrayList<VanillaOption>();

        Double fridayClose = quote.getOhlc().getClose();

        double lowerLimits = standardDeviation * bound;
        double strikeInterval = getStrikeInterval(fridayClose);
        MutableDouble shortPutStrike = new MutableDouble( getLowerBound(fridayClose,lowerLimits,strikeInterval)  );
        MutableDouble shortCallStrike = new MutableDouble( getUpperBound(fridayClose,lowerLimits,strikeInterval) );
        ...
        verifyCorrectCall(fridayClose,shortCallStrike,longCallStrike,strikeInterval);
        verifyCorrectPut(fridayClose, shortPutStrike, longPutStrike, strikeInterval);
    ...
    public IronCondorCurrent buildIronCondor(List<VanillaOption> options,Quote quote,double boundMultipleForSD, int daysForSD){
        OHLC condorOHLC = new OHLC();

        IronCondorCurrent condor = new IronCondorCurrent();
    ...
    private int getOpen(List<VanillaOption> list,IronCondorCurrent condor){
        try{
