...
import code.com.corybill.helper.CurrentICHelper;
 ...
 */
public class CurrentICDataLoad implements IDataLoad {
    private static Logger log = Logger.getLogger(CurrentICDataLoad.class);

    private MathUtil mathUtil;
    private CalendarHelper calendarHelper;
    private CurrentICHelper dlHelper;
    ...
    List<IronCondorCurrent> condors;

    //List to be saved to mongoDao
    List<Quote> quotesQuery;
    Map<String,List<VanillaOption>> optionsByExpiration;
    ...
    public void invoke() {
        ...
        for(String symbol : listOfSymbols){
            ...
            condors = new ArrayList<IronCondorCurrent>();
            ...
            icCurrentDao.saveIronCondors(condors);
    ...
    public void getData(String symbol,double... doubles) {
        try{
            if(quotesQuery.size() > 0){
                ...
                List<VanillaOption> options = dlHelper.createCondorsOptions(quote,standardDeviation,doubles[1]);

                IronCondorCurrent condor = dlHelper.buildIronCondor(options, quote, doubles[1], (int) doubles[0]);
    ...
    public CurrentICHelper getDlHelper() {
        return dlHelper;
    }
    public void setDlHelper(CurrentICHelper dlHelper) {
